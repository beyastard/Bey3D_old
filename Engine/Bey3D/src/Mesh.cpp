...
ModelException::ModelException(int line, const char* file, std::string note) noexcept
	: BeyException(line, file), note(std::move(note))
{}

const char* ModelException::what() const noexcept
{
	std::ostringstream oss;
	oss << BeyException::what() << std::endl << "[Note] " << GetNote();
	whatBuffer = oss.str();
...
Node::Node(int id, const std::string& name, std::vector<Mesh*> meshPtrs, const DirectX::XMMATRIX& transform_in) noxnd
	: name(name), id(id), meshPtrs(std::move(meshPtrs))
{
	XMStoreFloat4x4(&transform, transform_in);
	XMStoreFloat4x4(&appliedTransform, dx::XMMatrixIdentity());
}
...
std::unique_ptr<Mesh> Model::ParseMesh(Graphics& gfx, const aiMesh& mesh)
{
	...
	Dynamic::VertexBuffer vbuf(std::move(VertexLayout{}.Append(VertexLayout::Position3D).Append(VertexLayout::Normal)));
